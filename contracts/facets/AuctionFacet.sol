...
error ZERO_DURATION_NOT_PERMITTED();
    ...
    function createAuction(uint _durationInSeconds, uint _startingBid, uint _nftTokenId) external{
        ...
        if (_durationInSeconds == 0) revert ZERO_DURATION_NOT_PERMITTED();

        if (_startingBid == 0) revert ZERO_PRICE_NOT_PERMITTED();

        // checking for owner of the NFT
        if (IERC721(l.nftContractAddress).ownerOf(_nftTokenId) != msg.sender) revert ONLY_NFT_OWNER();
    ...
    function bid(uint _auctionId, uint _bid) external returns (address highestBidder_) {
    ...
    function claimTokenEqOfAuctionItem(uint _auctionId) external {
        ...
        if (ad.auctionCreator != msg.sender) revert ONLY_TOKEN_OWNER();
